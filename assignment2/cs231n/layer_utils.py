...
def affine_norm_relu_forward(x, w, b, gamma, beta, norm, bn_param):
    """Convenience layer that performs an affine transform followed by normalization and a ReLU.

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - bn_params: Parameters needed for normalization

    Returns a tuple of:
    - out: Output from the norm
    - cache: Object to give to the backward pass
    """
...
def affine_norm_relu_backward(dout, cache, norm):
    ...
    if norm == "batchnorm":
      ...
      dnorm, dgamma, dbeta = layernorm_backward(drelu, norm_cache)
